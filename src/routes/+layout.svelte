...
<svelte:head>
	...
	{:else if (config['status-website'] || {}).theme}
		<link rel="stylesheet" href={`${config.path}/themes/${config['status-website'].theme}.css`} />
	{:else}
	...
	{#if (config['status-website'] || {}).scripts}
		{#each (config['status-website'] || {}).scripts as script}
			<script src={script.src} async={!!script.async} defer={!!script.async}></script>
		{/each}
	{/if}
	...
	{#if (config['status-website'] || {}).metaTags}
		{#each (config['status-website'] || {}).metaTags as meta}
